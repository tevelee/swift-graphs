...
struct VisitorPatternTests {
    ...
    @Test func dfsPreorder() {
        ...
        let result = graph.traverse(
            from: a,
            using: .dfs(order: .preorder).withVisitor(
                .init(
    ...
    @Test func dfsPostorder() {
        ...
        let result = graph.traverse(
            from: a,
            using: .dfs(order: .postorder).withVisitor(
                .init(
    ...
    @Test func dfsDepthLimited() {
        ...
        _ = graph.traverse(
            from: a,
            using: .depthLimitedDFS(maxDepth: 2).withVisitor(
                .init(
                    discoverVertex: { vertex in
                        visitedVertices.append(graph[vertex].label)
                    }
    ...
    // MARK: - Shortest Path
        ...
        @Test func dijkstra() {
            ...
            var visitedVertices: [String] = []
            let result = graph.shortestPath(
                from: a,
                to: c,
                using: .dijkstra(weight: .property(\.weight)).withVisitor(
                    .init(
                        examineVertex: { vertex in
                            visitedVertices.append(graph[vertex].label)
                        }
        ...
        @Test func aStar() {
            ...
            let result = graph.shortestPath(
                from: a,
                to: c,
                using: .aStar(weight: .property(\.weight), heuristic: .uniform(0.0)).withVisitor(
                    .init(
                        examineVertex: { vertex in
                            visitedVertices.append(graph[vertex].label)
                        }
        ...
        @Test func bellmanFord() {
            ...
            let result = graph.shortestPath(
                from: a,
                to: c,
                using: .bellmanFord(weight: .property(\.weight)).withVisitor(
                    .init(
                        examineEdge: { edge in
    ...
    // MARK: - Minimum Spanning Tree
        ...
        @Test func kruskal() {
            ...
            var visitedEdges: [String] = []
            let result = graph.minimumSpanningTree(
                using: .kruskal(weight: .property(\.weight)).withVisitor(
                    .init(
                        examineEdge: { edge in
        ...
        @Test func prim() {
            ...
            var visitedEdges: [String] = []
            let result = graph.minimumSpanningTree(
                using: .prim(weight: .property(\.weight)).withVisitor(
                    .init(
                        examineEdge: { edge in
                            let source = graph.source(of: edge)!
                            let dest = graph.destination(of: edge)!
    ...
    // MARK: - Graph Coloring
        ...
        @Test func greedyColoring() {
            ...
            var visitedVertices: [String] = []
            let result = graph.colorGraph(
                using: .greedy().withVisitor(
                    .init(
                        examineVertex: { vertex in
                            visitedVertices.append(graph[vertex].label)
                        }
    ...
    // MARK: - Hamiltonian Path
        ...
        @Test func backtrackingHamiltonian() {
            ...
            var visitedVertices: [String] = []
            let result = graph.hamiltonianPath(
                using: .backtracking().withVisitor(
                    .init(
                        examineVertex: { vertex in
                            visitedVertices.append(graph[vertex].label)
                        }
        ...
        @Test func hierholzer() {
            ...
            var visitedVertices: [String] = []
            let result = graph.eulerianPath(
                using: .hierholzer().withVisitor(
                    .init(
                        examineVertex: { vertex in
                            visitedVertices.append(graph[vertex].label)
                        }
        ...
        @Test func vf2Isomorphism() {
            ...
            var visitedVertices: [String] = []
            let result = graph1.isIsomorphic(
                to: graph2,
                using: .vf2().withVisitor(
                    .init(
                        examineVertex: { vertex in
    ...
    // MARK: - Additional Shortest Path
        ...
        @Test func floydWarshall() {
            ...
            let result = graph.shortestPath(
                from: a,
                to: c,
                using: .floydWarshall(weight: .property(\.weight)).withVisitor(
                    .init(
                        examineVertex: { vertex in
                            visitedVertices.append(graph[vertex].label)
                        }
        ...
        @Test func johnson() {
            ...
            let result = graph.shortestPath(
                from: a,
                to: c,
                using: .johnson(weight: .property(\.weight)).withVisitor(
                    .init(
        ...
        @Test func yen() {
            ...
            let result = graph.shortestPath(
                from: a,
                to: c,
                using: .yen(weight: .property(\.weight)).withVisitor(
                    .init(
                        onPathFound: { path in
