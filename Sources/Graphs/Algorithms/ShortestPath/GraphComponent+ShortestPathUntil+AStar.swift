...
extension ShortestPathUntilAlgorithm {
    ...
    /// - Returns: An instance of `AStarUntilAlgorithm`.
    ...
    ) -> Self where Self == AStarUntilAlgorithm<Node, Edge, Distance, Cost> {
    ...
    @inlinable public static func aStar<Node, Edge: Weighted>(
        heuristic: Self.Heuristic
    ) -> Self where Self == AStarUntilAlgorithm<Node, Edge, Edge.Weight, Edge.Weight> {
    ...
    @inlinable public static func aStar<Node, Edge: Weighted, Distance: BinaryFloatingPoint>(
        heuristic: Self.Heuristic
    ) -> Self where Self == AStarUntilAlgorithm<Node, Edge, Distance, Distance>, Edge.Weight: BinaryInteger {
        .init(heuristic: heuristic) { Distance($0) + $1 }
    }
}

extension AStarUntilAlgorithm.Heuristic where HScore: FloatingPoint, HScore.Magnitude == HScore {
    /// Creates a heuristic based on the Euclidean distance.
    /// - Parameter value: A closure to extract the coordinates from a node.
    /// - Returns: An instance of `AStarUntilAlgorithm.Heuristic`.
    @inlinable public static func euclideanDistance<Coordinate: SIMD>(
        of value: @escaping (Node) -> Coordinate,
        towards destination: Node
    ) -> Self where HScore == Coordinate.Scalar {
        self.init(distanceAlgorithm: .euclideanDistance(of: value), towards: destination)
    }
    ...
    @inlinable public static func manhattanDistance<Coordinate: SIMD>(
        ...
        self.init(distanceAlgorithm: .manhattanDistance(of: value), towards: destination)
    }
}

/// An implementation of the A* algorithm for finding the shortest path in a graph.
public struct AStarUntilAlgorithm<Node: Hashable, Edge: Weighted, HScore, FScore: Comparable>: ShortestPathUntilAlgorithm, ShortestPathAlgorithm where Edge.Weight: Numeric {
    /// A heuristic function used by the A* algorithm.
    public struct Heuristic {
        /// A closure to estimate the distance between two nodes.
        public let estimatedDistance: (Node) -> HScore
        ...
        @inlinable public init(estimatedDistance: @escaping (Node) -> HScore) {
        ...
        @inlinable public init(distanceAlgorithm: DistanceAlgorithm<Node, HScore>, towards destination: Node) where HScore.Magnitude == HScore {
            self.init { distanceAlgorithm.distance($0, destination) }
        }
    }
    ...
    @inlinable public func shortestPath(
        ...
        openSet.insert(
            State(
                node: source,
                costSoFar: .zero,
                estimatedTotalCost: calculateTotalCost(.zero, heuristic.estimatedDistance(source))
            )
        )
        ...
        while let currentState = openSet.popMin() {
            ...
            for edge in graph.edges(from: currentNode) {
                ...
                if costs[neighbor] == nil || newCost < costs[neighbor]! {
                    ...
                    let estimatedTotalCost = calculateTotalCost(newCost, heuristic.estimatedDistance(neighbor))
