...
public struct YensAlgorithm<Node: Hashable, Edge: Weighted & Equatable>: KShortestPathsAlgorithm where Edge.Weight: Numeric, Edge.Weight.Magnitude == Edge.Weight {
    ...
    @inlinable public func kShortestPaths(
        ...
        if let shortestPath = graph.shortestPath(
            from: source,
            to: destination,
            using: .dijkstra()
        ) {
