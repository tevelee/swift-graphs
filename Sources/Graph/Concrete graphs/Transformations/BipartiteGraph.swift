/// A bipartite graph.
public struct BipartiteGraph<Base: GraphProtocol>: BipartiteGraphProtocol where Base.Node: Hashable {
...
extension GraphProtocol where Node: Hashable {
    /// Returns a bipartite graph with the given left and right partitions.
    /// - Parameters:
    ///  - leftPartition: The left partition of the bipartite graph.
    ///  - rightPartition: The right partition of the bipartite graph.
    /// - Returns: A `BipartiteGraph` instance.
    @inlinable public func bipartite(leftPartition: Set<Node>, rightPartition: Set<Node>) -> BipartiteGraph<Self> {
