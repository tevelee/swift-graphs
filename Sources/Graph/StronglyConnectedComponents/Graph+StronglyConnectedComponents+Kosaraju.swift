...
public struct KosarajuSCCAlgorithm<Node: Hashable, Edge>: StronglyConnectedComponentsAlgorithm {
    ...
    @inlinable public func findStronglyConnectedComponents(in graph: some WholeGraphProtocol<Node, Edge>) -> [[Node]] {
    ...
    @usableFromInline func dfsCollect(graph: some WholeGraphProtocol<Node, Edge>, node: Node, visited: inout Set<Node>, scc: inout [Node]) {
        visited.insert(node)
        scc.append(node)
    ...
    @usableFromInline func reverseGraph(_ graph: some WholeGraphProtocol<Node, Edge>) -> Graph<Node, Edge> {
